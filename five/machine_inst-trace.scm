...
;;5.2.1: machine-model
;;--------------------------------------------------------
;; trace-flag: "trace-on" or "trace-off"
(define (make-machine register-names ops trace-flag controller-text)
  (let ((machine (make-new-machine)))
   (for-each ; 新しいマシンの中にレジスタを割りあてる
     (lambda (register-name)
       ((machine 'allocate-register) register-name))
     register-names)
   ((machine 'install-operations) ops)
   ((machine 'trace-flag) trace-flag)
    ...
    (let ((the-ops
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'trace-flag) trace-flag)
