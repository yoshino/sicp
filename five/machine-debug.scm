      ...
      (define (execute)
        (let ((insts (get-contents pc)))
         (if (null? insts)
             'done
             (begin
               (if (not (eq? (caar insts) 'label)) ;;labelごとにリセットしたいので
                   (set! count-from-label (+ count-from-label 1))
                   (set! count-from-label 0))
               (set! count-from-label (+ count-from-label 1))
               (if (member (cons label count-from-label) brerakpoints) ;;分岐を追加
                   (display 'DEBUG)
                   (begin
                     ((instruction-execution-proc (car insts))) ; 直列に命令を実行している
                     (execute)))))))
      (define (set-bpoint label n)
        (if (not (memq (cons label n) (map car brerakpoints)))
            (set! brerakpoints (cons (cons label n) brerakpoints))))
      (define (del-bpoint label n)
        (if (member (cons label n) brerakpoints)
            (set! brerakpoints (delete (cons label n) brerakpoints))
